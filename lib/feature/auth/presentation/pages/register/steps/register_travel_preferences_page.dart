...
class RegisterTravelPreferencesPage extends StatefulWidget {
  const RegisterTravelPreferencesPage({super.key});

  @override
  State<RegisterTravelPreferencesPage> createState() => _RegisterTravelPreferencesPageState();
}

class _RegisterTravelPreferencesPageState extends State<RegisterTravelPreferencesPage> {

  final List<TravelPreference> _preferenceList = [];
  
  @override
  void initState() {
    super.initState();
    context.read<TravelPreferencesCubit>().load();
  }
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: SizedBox.expand(
            child: BlocBuilder<TravelPreferencesCubit, TravelPreferencesState>(
                ...
                return Column(
                  ...
                  children: [
                    ...
                    SizedBox(
                      width: 348.w,
                      child: Text(
                        "Personalize suas preferências de viagem e nós adaptaremos as recomendações ao seu estilo. Você sempre pode mudar isso. 👍",
                        style: TextStyle(
                          color: AppColors.darkTextColor,
                          fontSize: 16.sp,
                          height: 1.15.w
                    ...
                    SizedBox(
                      ...
                      child: SingleChildScrollView(
                        child: Wrap(
                          ...
                          runSpacing: 8,
                          children: [
                            if (state is TravelPreferencesLoaded)
                              ...List.generate(
                                state.preferences.length,
                                (index) {
                                  final preference = state.preferences[index];
                                  final isOnList = _preferenceList.any((pref) => pref.id == preference.id);
                                  ...
                                  return TweenAnimationBuilder<double>(
                                    ...
                                    child: ChipWidget(
                                      string: "${preference.emoji} ${preference.category}",
                                      isSelected: isOnList,
                                      selectedBorderColor: AppColors.lightBlue,
                                      unselectedBorderColor: AppColors.lightBorderColor,
                                      onTap: (){
                                        if (isOnList) {
                                          setState(() {
                                            _preferenceList.removeWhere((pref) => pref.id == preference.id);
                                          });
                                        } else {
                                          if (!isFull) {
                                            setState(() {
                                ...
                                (index) => ChipShimmerWidget(
                                  width: 60.w + (index % 3) * 20.w, // larguras variadas
                                ),
                              ),
                          ],
                        ),
                      ...
                      builder: (context, state) {
                        ...
                        return SizedBox(
                          ...
                          child: ElevatedButton(
                            ...
                            ? (){
                              final _idList = _preferenceList.map((p) => p.id).toList();
                              context.read<RegisterBloc>().add(RegisterSubmitTravelPrefsRequested(_idList));
                            }
                            : null,
                            style: ButtonStyle(
